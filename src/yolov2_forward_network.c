    ...
    for (i = 0; i < l.batch; ++i)
    {
        ...
        if (l.xnor && l.align_bit_weights && (l.stride == 1 && l.pad == 1))
        {
            ...
            if (l.c % 32 == 0)
            {
                ...
                gemm_nn_custom_bin_mean_transposed(m, n, k, 1, l.align_bit_weights, new_ldb, t_bit_input, new_ldb, c, n,
                                                   l.mean_arr);
        ...
        for (b = 0; b < l.batch; b++)
        {
            for (f = 0; f < l.out_c; ++f)
            {
                for (i = 0; i < out_size; ++i)
                {
                    ...
                    l.output[index + b * l.outputs] = (l.output[index + b * l.outputs] - l.rolling_mean[f]) /
                                                      (sqrtf(l.rolling_variance[f]) + .000001f);
                }
            }
...
float *network_calibrate_cpu(network net, float *input)
{
    ...
    if (net.do_input_calibration > 0)
    {    // calibration for quantinization
